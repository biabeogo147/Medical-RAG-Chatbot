...
| Step 17 test: `root` stays `Progressing`, the message says `Retrying attempt #N` | Normal for a few minutes: Argo CD retries an automated sync five times before it marks it `Failed` (step 15, *Retries come first*). Wait for it with point 3 of the test |
| Step 17 test: the Job says `but 0 was expected` | The test version was written without quotes, so YAML read twelve zeros as the number 0. The test still failed as intended; for the expected message, quote it |
| Step 16: the Job fails on Hugging Face (`429`, `503`) | The quota or the service. The Job has no second attempt on purpose. Wait, then start one sync by hand with the `kubectl patch` of step 17 point 7 (or **Sync** in the Argo CD UI). A new commit does not help unless it changes the rendered objects |
| After a fix, `root` stays `Degraded` and the app's last sync stays `Failed` | The fix made the rendered objects equal the live ones, so the app is `Synced` and automated sync does not run. Start one sync by hand (step 17 point 7) |
