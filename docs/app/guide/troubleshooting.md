...
| Step 4: the `sa.pub` hashes differ between nodes | A join did not receive the key through `--upload-certs`. Look at `kubeadm_join` in the `make cluster` output for that node. Rebuild; do not copy the key by hand |
