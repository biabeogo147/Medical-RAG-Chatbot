...
|---|---|---|---|
| [1](guide/1-argocd.md) | [1](guide/1-argocd.md#step-1--cluster-up-tunnel-open-secrets-present) | Cluster up, tunnel open, secrets present | 3 nodes `Ready`; the Rancher and alertmanager secrets have a current version |
| [1](guide/1-argocd.md) | [2](guide/1-argocd.md#step-2--install-argo-cd-by-hand) | Argo CD installed by hand | Every Argo CD pod `Running`; the API answers `ok` |
| [1](guide/1-argocd.md) | [3](guide/1-argocd.md#step-3--the-root-application-and-argo-cd-managing-itself) | `root` app-of-apps, Argo CD manages itself, `make bootstrap` | `argocd` and `root` are `Synced` and `Healthy` |
| [2](guide/2-foundation.md) | [4](guide/2-foundation.md#step-4--ingress-nginx) | ingress-nginx | Both ingress target groups `healthy`; the public NLB answers `404` |
| [2](guide/2-foundation.md) | [5](guide/2-foundation.md#step-5--ebs-csi-driver-and-the-gp3-storageclass) | EBS CSI driver + `gp3` StorageClass | A test volume is created, used and deleted |
| [2](guide/2-foundation.md) | [6](guide/2-foundation.md#step-6--external-secrets-and-the-platform-secrets) | External Secrets + `platform-secrets` | `tls-rancher-ingress` exists, with the right certificate |
| [3](guide/3-certificates-and-argocd-ui.md) | [7](guide/3-certificates-and-argocd-ui.md#step-7--cert-manager-and-the-wildcard-certificate) | cert-manager + wildcard certificate | `*.recruitai.io.vn` `READY True`, issued by Let's Encrypt production |
| [3](guide/3-certificates-and-argocd-ui.md) | [8](guide/3-certificates-and-argocd-ui.md#step-8--keep-the-certificate-across-rebuilds) | Certificate backup and restore | The certificate is in Secrets Manager; the restore object is `SecretSynced` |
| [3](guide/3-certificates-and-argocd-ui.md) | [9](guide/3-certificates-and-argocd-ui.md#step-9--the-argo-cd-ui-through-the-vpn) | Argo CD UI | `argocd.recruitai.io.vn`: no VPN → timeout; VPN → login works |
| [4](guide/4-monitoring-and-rancher.md) | [10](guide/4-monitoring-and-rancher.md#step-10--monitoring-the-whole-cluster-alert-email-three-uis) | Monitoring, alert email, 3 UIs | Every target `up` incl. etcd; a test alert arrives by email; Grafana, Prometheus, Alertmanager open through the VPN |
| [4](guide/4-monitoring-and-rancher.md) | [11](guide/4-monitoring-and-rancher.md#step-11--rancher) | Rancher | Criterion #4: no VPN → timeout; VPN → `pong`; the chain verifies; the UI loads |
| [5](guide/5-teardown-and-rebuild.md) | [12](guide/5-teardown-and-rebuild.md#step-12--make-down) | `make down` by hand, then as a target | No CSI volume left in EC2, cluster destroyed |
| [5](guide/5-teardown-and-rebuild.md) | [13](guide/5-teardown-and-rebuild.md#step-13--rebuild-from-nothing-and-the-evidence) | Rebuild from nothing, evidence | Every Application `Synced` and `Healthy`; no new certificate ordered; times recorded |
