...
| 19 | Workstation | Internal UI names, cert-manager DNS permission, alert email and certificate-backup secrets | 19 resources in `shared`, 88 in `cluster`; `argocd.<domain>` resolves to private addresses |
...
## Part E — Internal UIs, their certificate, and alert email

Part E prepares AWS for the GitOps phase. Rule for the whole project: **every UI of an internal tool
